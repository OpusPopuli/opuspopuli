# Commercial Licensing

The Opus Populi platform is available under two licensing options:

1. **Open Source License (AGPL-3.0)** - Free for open source and non-commercial use
2. **Commercial License** - For proprietary/commercial applications

## Open Source License (AGPL-3.0)

The default license for Opus Populi is the [GNU Affero General Public License v3.0](LICENSE).

Under AGPL-3.0, you can freely:
- Use the platform for any purpose
- Modify the source code
- Distribute the platform
- Run it as a network service

**Requirements:**
- Disclose source code of modifications
- License modifications under AGPL-3.0
- Provide source code to users accessing the service over a network
- Preserve copyright notices and license information

This license is ideal for:
- Non-profit organizations
- Government agencies
- Educational institutions
- Open source projects
- Organizations comfortable with source code disclosure

## Commercial License

A commercial license is available for organizations that:
- Cannot comply with AGPL-3.0 source disclosure requirements
- Want to incorporate the platform into proprietary products
- Need additional support and services
- Require indemnification or warranty coverage

### Commercial License Benefits

| Feature | AGPL-3.0 | Commercial License |
|---------|----------|-------------------|
| Source code disclosure | Required | Not required |
| Modification disclosure | Required | Not required |
| Use in proprietary products | Limited | Unlimited |
| Warranty | None | Available |
| Indemnification | None | Available |
| Priority support | Community only | Dedicated support |
| Custom development | Not included | Available |
| Training | Not included | Available |

### License Tiers

#### Startup License

For small organizations and startups.

**Eligibility:**
- Less than $1M annual revenue
- Fewer than 20 employees

**Includes:**
- Commercial license for one deployment
- Email support (48-hour response)
- Quarterly security updates

**Pricing:** Contact for quote

---

#### Standard License

For established organizations with production deployments.

**Includes:**
- Commercial license for up to 5 deployments
- Priority email support (24-hour response)
- Monthly security updates
- Access to private security advisories
- 2 hours/month technical consultation

**Pricing:** Contact for quote

---

#### Enterprise License

For large organizations with complex requirements.

**Includes:**
- Commercial license for unlimited deployments
- Dedicated support channel (4-hour response)
- Weekly security updates
- Custom feature development (negotiated)
- On-site training (negotiated)
- SLA with uptime guarantees
- Legal indemnification

**Pricing:** Contact for quote

---

### Add-On Services

Available with any commercial license:

| Service | Description |
|---------|-------------|
| Custom Development | Feature development specific to your needs |
| Migration Assistance | Help migrating from other platforms |
| Training | On-site or remote training for your team |
| Architecture Review | Expert review of your deployment architecture |
| Security Audit | Third-party security assessment |
| Performance Tuning | Optimization for high-traffic deployments |

## Support Options

### Community Support (Free)

Available to all users:
- GitHub Issues
- GitHub Discussions
- Community documentation
- Stack Overflow (tag: `opuspopuli`)

### Commercial Support

Available to commercial license holders:

| Tier | Response Time | Channels | Hours |
|------|---------------|----------|-------|
| Standard | 24 hours | Email, GitHub | Business hours |
| Priority | 4 hours | Email, Slack | Business hours |
| Enterprise | 1 hour | Dedicated channel | 24/7 |

## Dual Licensing FAQ

### Can I use AGPL-3.0 for commercial purposes?

Yes, but you must comply with AGPL-3.0 requirements, including making your source code available to users who access your service over a network.

### When do I need a commercial license?

You need a commercial license if you:
- Cannot or don't want to disclose your source code
- Want to embed the platform in a proprietary product
- Need warranty or indemnification coverage
- Require dedicated support with SLA guarantees

### Can I switch from AGPL-3.0 to commercial later?

Yes. You can start with AGPL-3.0 and upgrade to a commercial license at any time. Contact us for migration assistance.

### What about contributions I make under AGPL-3.0?

Contributions to the upstream repository are made under AGPL-3.0 with a Contributor License Agreement (CLA). This allows Opus Populi to offer commercial licensing while keeping the project open source.

### Does the commercial license apply to my custom code?

The commercial license covers the Opus Populi platform code. Your custom code (region providers, custom features) can be licensed however you choose.

### Can I get a trial commercial license?

Yes. We offer 90-day evaluation licenses for organizations considering commercial licensing. Contact us for details.

## Contributor License Agreement

Contributors to the Opus Populi platform sign a CLA that:
- Grants Opus Populi the right to sublicense contributions
- Allows Opus Populi to offer commercial licenses
- Preserves the contributor's rights to their contributions
- Ensures the project remains open source under AGPL-3.0

This dual-licensing model:
- Keeps the platform 100% open source
- Funds ongoing development and maintenance
- Provides options for commercial users
- Supports the civic technology mission

## Contact

For commercial licensing inquiries:

**Email:** licensing@opuspopuli.org

**Request a Quote:**
Please include:
- Organization name and type
- Number of deployments needed
- Expected user count
- Any specific requirements
- Timeline for deployment

**Response Time:** Within 2 business days

---

## Legal Notices

### Trademark

"Opus Populi," "Opus Populi," and associated logos are trademarks of Opus Populi. Commercial license holders receive limited trademark rights as specified in their license agreement.

### Export Compliance

The platform may be subject to export control laws. Commercial licensees are responsible for ensuring compliance with applicable export regulations.

### Third-Party Components

The platform includes third-party open source components. Commercial licenses do not modify the licenses of these components. See [NOTICE](NOTICE) for third-party license information.

---

*Last updated: January 2025*
