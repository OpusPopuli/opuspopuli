...
function SourceAttribution({
  ...
  return (
    <div className="mt-4 pt-3 border-t border-line flex items-center justify-between text-[11px] text-content-dim">
      <span>
        Source:{" "}
        <a
          href={source.url}
  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-3">
        ...
        {isAi && (
          <span
            className="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full bg-warning-surface text-warning border border-warning-line"
      ...
      {isAi && (
        <div className="mt-3 flex items-baseline gap-3 text-[11px]">
          ...
          <button
            ...
            className="text-content-dim hover:text-content hover:underline whitespace-nowrap"
      ...
      {isAi && showAttribution && (
        <div id="bio-attribution" className="mt-4 pt-4 border-t border-line">
          <h4 className="text-xs font-semibold uppercase tracking-wider text-content mb-3">
...
function Committees({
  ...
  return (
    <div className="mb-8">
      ...
      {summary && (
        <div className="mb-5 p-4 bg-warning-surface/40 border-l-4 border-warning-line rounded-r">
          <div className="flex items-start justify-between gap-3 mb-1">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-warning">
              At a glance
            </h4>
            <span
              className="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full bg-warning-surface text-warning border border-warning-line flex-shrink-0"
...
function BioClaims({ claims }: { readonly claims?: readonly BioClaim[] }) {
  ...
  return (
    <ol className="space-y-3">
      {claims.map((c, idx) => (
        <li key={c.sentence} className="text-sm border-l-2 border-line pl-3">
          <div className="flex items-baseline gap-2">
            <span className="text-xs font-mono text-content-dim flex-shrink-0">
              {idx + 1}.
            </span>
          ...
          <div className="mt-1 ml-5 flex flex-wrap items-center gap-2 text-[11px]">
            {c.origin === "source" ? (
              <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-positive-surface text-positive border border-positive-line font-medium">
                Source
              </span>
            ) : (
              <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-warning-surface text-warning border border-warning-line font-medium">
                Training knowledge
              </span>
            )}
            ...
            {c.origin === "training" && c.sourceHint && (
              <span className="text-content-dim italic">— {c.sourceHint}</span>
            )}
            {c.confidence && (
      ...
      <li className="text-[11px] text-content-dim italic pt-2 border-t border-line">
...
function RepBillsList({
  ...
  if (loading && !data) {
    return (
      <div className="space-y-2 animate-pulse">
        {[1, 2, 3].map((n) => (
          <div key={n} className="h-12 bg-surface-alt rounded-lg" />
        ))}
      </div>
    );
  }
  ...
  if (bills.length === 0) {
    return <p className="text-sm italic text-content-dim">{emptyMessage}</p>;
  }
...
function ContactChip({
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-1.5 text-xs text-content-dim hover:text-info-strong hover:underline max-w-[220px] min-h-[24px]"
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          {contact?.email && (
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-on-inverse bg-inverse-surface rounded-lg hover:opacity-90 transition-colors"
            >
              Contact {firstName}
            </button>
          )}
