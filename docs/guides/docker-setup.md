...
**Production deploy lives in a separate repo.** `docker-compose-prod.yml` and its bind-mount sources moved to the per-region deployment template at [`OpusPopuli/opuspopuli-node`](https://github.com/OpusPopuli/opuspopuli-node). Each region operator forks that template and runs `docker compose -f docker-compose-prod.yml pull && up -d` on their own Mac Studio. The Studio pulls cosign-signed images from `ghcr.io/opuspopuli/*` (published by `.github/workflows/release.yml` here on push to `main`); no `pnpm install` or `docker compose build` runs on the Studio. See [`container-verification.md`](container-verification.md) for the image-verification procedure.
